...
extern "C" int onemklSrotmg(syclQueue_t device_queue, float *d1, float *d2, float *x1, float *y, float *param) {
   auto status = oneapi::mkl::blas::column_major::rotmg(device_queue->val, d1, d2, x1, y, param, {});
   __FORCE_MKL_FLUSH__(status);
   return 0;
}

extern "C" int onemklDrotmg(syclQueue_t device_queue, double *d1, double *d2, double *x1, double *y, double *param) {
   auto status = oneapi::mkl::blas::column_major::rotmg(device_queue->val, d1, d2, x1, y, param, {});
   __FORCE_MKL_FLUSH__(status);
   return 0;
}
